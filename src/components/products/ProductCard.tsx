...
export const ProductCardSkeleton = ({
  ...
  return <div className={cn("group w-full", className)}>
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    try {
      ...
      logFormSuccess('cart', 'add_to_cart_success', {
        ...
        productTitle: product.title,
        price: {
          original: firstSize.priceOriginal,
          discounted: firstSize.priceDiscounted
        }
      });
    } catch (error) {
  ...
  return <div className={cn("group w-full", className)}>
      <div className="aspect-square overflow-hidden relative rounded-md mb-2 shadow-lg">
        <Link to={`/product/${product.slug}`} onClick={handleProductClick}>
          <img src={thumbnailImage || "/placeholder.svg"} alt={product.title} className="w-full h-full object-cover" onError={e => {
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-2">
          {hasDiscount && <span className="bg-red-500 text-white text-xs font-semibold px-2 py-1 rounded">
              Offer
            </span>}
          {product.isOutOfStock && <span className="bg-gray-700 text-white text-xs font-semibold px-2 py-1 rounded">
              Out of Stock
            </span>}
        </div>
        ...
        <div className="absolute right-2 top-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <Button variant="secondary" size="icon" className="bg-white text-gray-700 hover:text-brand shadow-sm" aria-label="Add to wishlist" onClick={handleAddToWishlist}>
            <Heart size={18} />
          </Button>
          
          {!product.isOutOfStock && <Button variant="secondary" size="icon" className="bg-white text-gray-700 hover:text-brand shadow-sm" aria-label="Add to cart" onClick={handleAddToCart} disabled={isAddingToCart}>
      ...
      {/* Product info */}
      <div>
        <Link to={`/product/${product.slug}`} onClick={handleProductClick} className="text-sm font-medium text-gray-700 hover:text-brand transition-colors mb-1 line-clamp-2">
          {product.title}
        </Link>
